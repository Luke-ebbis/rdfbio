...
pub mod csv {
    ...
    pub fn write_omicsdi(
        object: OmicsDiResponse,
        out: Option<String>,
        ...
        let writer: Box<dyn Write> = match out {
            Some(name) => {
                let mut file = OpenOptions::new()
                    ...
                    .create(true)
                    .open(&name)
                    .expect(&format!("Cannot open {}", name));
                Box::new(file)
            }
        ...
        wtr.write_record(&["identifier", "title", "type", "description", "organisms"])?;
